...
function ProductCart({ product, stringColSpan }: props) {
  const convertName = (namePhone: string) => {
    const arrLink = namePhone.toLowerCase().split(' ')
    return arrLink.join('-')
  }

  console.log(stringColSpan)

  return (
    <div className={`${stringColSpan} bg-white rounded-xl shadow-md mb-3 relative group overflow-hidden`}>
