...
function ProductCart({ product, className }: props) {
  ...
  return (
    <div
      className={`${className} bg-white rounded-xl shadow-md mb-3 relative group overflow-hidden flex-col justify-between`}
    >
      ...
      <div className='p-2'>
        <div className='paragraph-container'>
          <Link
            to={`${(() => {
              if (product.category_id == 1) return '/dien-thoai/'
              else if (product.category_id == 2) return '/laptop/'
              else if (product.category_id == 3) return '/phu-kien/'
              else if (product.category_id == 4) return '/smart-watch/'
              else return '/tablet/'
