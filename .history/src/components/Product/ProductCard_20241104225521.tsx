...
import formatToVND, { convertName, formatUrlLinkProduct } from '../../helpers/currencyFormatter'
interface props {
  product: product
  className: String
}
function ProductCart({ product, className }: props) {
  ...
  return (
    <div
      className={`${className}  rounded-xl shadow-md mb-3 relative group overflow-hidden bg-red-400 w-full grid grid-cols-6 `}
    >
      ...
      <div className='mt-2 overflow-hidden group p-2 col-span-6 mb-auto'>
        <img className='w-full h-full group-hover:scale-110 duration-500 ease-out' src={product.urlImage} alt='' />
      </div>
      <div className='p-2 bg-yellow-400 col-span-6 h-max mt-auto'>
