...
function HistoryView(props: props) {
  ...
  return (
    <div className='container bg-history-view bg-cover py-3 rounded-lg w-full'>
          ...
          historyProduct.map((p: any, idx) => {
            return (
              <SwiperSlide className='col-span-3 ' key={idx}>
                <div className='relative col-span-3 grid grid-cols-12 gap-4 border border-gray-300 p-2 rounded-lg bg-white'>
                  <Link to={formatUrlLinkProduct(p.categoryName, p.name)} className='col-span-4 w-[60px] h-[60px]'>
                    <img className='object-cover w-full h-full' src={p.img} alt='' />
                  </Link>
                  <Link
                  ...
                  <button
                    ...
                    className='z-50 block text-gray-400 absolute top-0 right-0 p-1 cursor-pointer hover:text-red-600'
                  >
                    <IoIosCloseCircle />
                  </button>
                </div>
