...
const SearchAndFilter: React.FC<SearchAndFilterProps> = () => {
  ...
  const [totalNumberPage, setTotalNumberPage] = useState<number>(0)
  ...
  const handleFilterProduct = async () => {
    try {
      ...
      if (fetchApi && fetchApi.status == 200) {
        ...
        setTotalNumberPage(Math.ceil(fetchApi.data.pageSize / 8))
      }
          ...
          <div className='bg-white p-4 rounded-lg shadow'>
            <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4'>
              {!isLoading ? (
                ...
                productFilters.slice((pageSelect - 1) * 8, (pageSelect - 1) * 8 + 8).map((p: product) => {
