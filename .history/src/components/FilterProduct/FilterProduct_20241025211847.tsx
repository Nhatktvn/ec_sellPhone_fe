...
const SearchAndFilter: React.FC<SearchAndFilterProps> = () => {
  ...
  const handleFilterProduct = async () => {
    try {
      ...
      if (fetchApi && fetchApi.status == 200) {
        setProductFilters(fetchApi.data)
      }
    } catch (error) {
      console.log(error)
    }
  ...
  const fetchPriceData = useCallback(
    debounce(() => {
      handleFilterProduct()
      // Gọi API hoặc fetch dữ liệu với giá trị value
      // fetch(`/api/products?price=${value}`).then(response => response.json()).then(data => console.log(data));
    }, 500),
    [] // Dependency array trống để giữ debounce không thay đổi khi component re-render
  )

  // Hàm xử lý sự thay đổi giá trị slider
  const handleSliderChange = (value: [number, number]) => {
    // const newPrice: [number, number] = event.target.value as [number, number];
    setPriceRange(value)
    fetchPriceData()
  }
  return (
          ...
          <div className='bg-white p-4 rounded-lg shadow'>
            <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4'>
              {productFilters &&
                ...
                productFilters.map((p: product, idx) => {
