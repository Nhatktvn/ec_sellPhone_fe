...
const SearchBar = () => {
  ...
  const containInputSearch = useRef(null)
  ...
  const handleClickOutside = (event) => {
    if (ref.current && !ref.current.contains(event.target)) {
      ...
      Xử lý logic khi người dùng click ra ngoài element ở đây
    }
  };
  useEffect(() => {
  ...
  return (
    <div className='w-[80%] mx-auto'>
      <div
        ref={containInputSearch}
        className='bg-white rounded-xl p-1 flex w-full relative'
        onMouseDown={() => setInputSearch('')}
      >
        <input
