...
import { restorePass } from '../../apis/auth.api'
...
const ModalRestorePasswordUser = (props: props) => {
  const [newPassword, setNewPassword] = useState<string>('')
  const handleRestorePass = async () => {
    try {
      const fetchRestorePass = await restorePass(props.idUser, newPassword)
      if (fetchRestorePass && fetchRestorePass.status == 200) {
              ...
              <div className='p-4 md:p-5 text-center w-full flex flex-col items-center'>
                ...
                <div className='w-full'>
                  <div className='flex flex-col gap-1 items-start w-full mb-3'>
                    <label htmlFor='username'>Đặt lại mật khẩu:</label>
                    <input type='text' name='name' className='border border-gray-300 py-2 px-2 rounded-md w-full' />
                  </div>
                </div>
                <div>
                  <button
...
export default ModalRestorePasswordUser
