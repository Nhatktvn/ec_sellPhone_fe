  ...
  const getListProduct = async () => {
    try {
      const rsGetProducts = await getProductFilter(
        `?category=${cate.id}&maxPrice=500000000${brandChoose ? `&brand=${brandChoose}` : ''}`
      )
    ...
    <div className='container p-0 my-10'>
      <div className='bg-orange p-3 font-bold text-white text-2xl rounded-t-lg flex gap-2 items-center justify-between'>
        ...
        <ul className='flex gap-4 items-center'>
          <li className='text-base font-normal cursor-pointer' onClick={() => setBrandChoose('')}>
            Tất cả
          </li>
          {listBrand &&
            listBrand.length > 0 &&
            listBrand.map((brand) => {
              return (
                <li
                  ...
                  className={`text-base font-normal cursor-pointer border-2 p-2 ${brandChoose === brand.name && 'border-white rounded-lg'}`}
                  key={brand.id}
                >
                  {brand.name}
                </li>
