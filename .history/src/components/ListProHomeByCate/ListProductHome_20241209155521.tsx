...
const ListProductHome = (cate: category) => {
  ...
  const [brnadChoose, setBrandChoose] = useState<string>('')
  useEffect(() => {
    getListProduct()
    getListBrand()
  }, [])
  const getListProduct = async () => {
    try {
      const rsGetProducts = await getProductFilter(`?category=${cate.id}&maxPrice=500000000&brand=${brnadChoose}`)
    ...
    <div className='container p-0 my-10'>
      <div className='bg-orange p-3 font-bold text-white text-2xl rounded-t-lg flex gap-2 items-center justify-between'>
        ...
        <ul className='flex gap-4'>
          <li className='text-base font-normal' onClick={() => setBrandChoose('0')}>
            Tất cả
          </li>
          {listBrand &&
            listBrand.length > 0 &&
            listBrand.map((brand) => {
              return (
                <li onClick={() => setBrandChoose(brand.name)} className='text-base font-normal' key={brand.id}>
                  {brand.name}
                </li>
              )
            })}
