  ...
  const getProfileUser = async () => {
    try {
      ...
      if (getProfileUser && getProfileUser.status == 200) {
        ...
        dispatch(login(getProfileUser.data))
        setDataUser(getProfileUser.data)
      }
    } catch (error) {
      throw error
    }
        ...
        <div className='lg:col-span-4 col-span-12 flex items-center justify-between h-full'>
          <div className='text-3xl text-white flex gap-2 items-center'>
            ...
            <a
              ...
              target='_blank'
              className='text-sm flex flex-col hover:text-[#ffda24] duration-150 cursor-pointer'
            >
              <span>Gọi ngay</span>
              <span className='font-bold'>+84378025713</span>
            </a>
          </div>
              ...
              <ul className='scroll-cart overflow-y-scroll p-1 h-[300px]'>
                {cart.cartItems && cart.cartItems.length > 0 ? (
                  cart.cartItems.map((item) => (
                    <Link
                      key={item.id}
                      to={`/san-pham/${convertName(item.name)}`}
