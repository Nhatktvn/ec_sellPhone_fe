...
function ChatBox(props: Props) {
  ...
  const navigatePage = (contentPage: string | undefined) => {
    switch (contentPage) {
      case contentPage?.includes('trang chủ'):
        navigation('/')
        break
      case contentPage?.includes('trang đăng nhập'):
        navigation('/dang-nhap')
        break
      case contentPage?.includes('trang đăng kí') || contentPage?.includes('trang đăng ký'):
        navigation('/dang-ki')
        break
      default:
        break
