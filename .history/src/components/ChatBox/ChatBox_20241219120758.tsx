  ...
  const handleFetchOpenAi = async (content: string) => {
    try {
      ...
      if (fetchApi && fetchApi.status === 200) {
        ...
        const newMsg: Message = {
          id: props.messages.length + 1,
          text: await responseContent(response.type, response.content, navigation),
          sender: 'other',
          timestamp: new Date().toISOString(),
...
export const responseContent = async (type: string, content: string, navigate: any) => {
    ...
    case 'checkstock':
      try {
        ...
        if (fetchStock && fetchStock.status == 200) {
          return fetchStock.data ? `Sản phẩm ${content} vẫn còn hàng` : `Sản phẩm ${content} đã hết hàng`
        }
      } catch (error: any) {
        if (error.response.status == 404) {
          return 'Sản phẩm không có trong cửa hàng'
        }
        return 'Lỗi khi kiểm tra tồn kho'
      }
      return 'Lỗi khi kiểm tra tồn kho'
    case 'question':
      return <p>{content}</p>
