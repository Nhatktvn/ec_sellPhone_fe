...
const ModalReview = ({ setShowModal, productId }: props) => {
  ...
  const handleSendReview = async () => {
    try {
      if (!productId || !starReview || !image) {
        return
      }
      const data: FormData = new FormData()
      data.append('image', image)
      data.append('idProduct', productId.toString())
      data.append('contentReview', contentReview)
      data.append('rate', starReview.toString())
      console.log(data)

      const rsSendReview = await addReview(data)
      console.log(rsSendReview)
      setShowModal(false)
