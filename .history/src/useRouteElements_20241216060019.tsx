import { useRoutes } from 'react-router-dom'
import Login from './pages/Login'
import Register from './pages/Register'
import MainLayout from './layouts/MainLayout'
import HomePage from './pages/HomePage'
import ProductList from './pages/ProductList'
import ProductDetail from './pages/ProductDetail'
import ForgotPassword from './pages/ForgotPassword'
import ResetPassword from './pages/ResetPassword'
import ActiveAccount from './pages/AcctiveAccount/ActiveAccount'
import ConfirmActive from './pages/AcctiveAccount/ConfirmActive'
import AdminLayout from './layouts/AdminLayout'
import ManageUser from './pages/Admin/ManageUser/ManageUser'
import ManageCategory from './pages/Admin/ManageCategory/ManageCategories'
import ManaegProduct from './pages/Admin/ManageProduct/ManageProduct'
import ManageOrder from './pages/Admin/ManageOrder/ManageOrder'
import ManageBrand from './pages/Admin/ManageBrand/ManageBrand'
import Page404 from './pages/404/Page404'
import AddNewProduct from './pages/Admin/ManageProduct/AddNewProduct'
import AddNewBrand from './pages/Admin/ManageBrand/AddNewBrand'
import ListCart from './pages/ListCart'
import SearchPage from './pages/SearchPage/SearchPage'
import ProfilePage from './pages/ProfilePage'
import PaymentSuccess from './pages/PageResultPayment/PaymentSuccess'
import PaymentFail from './pages/PageResultPayment/PaymentFail'
import DetailOrderPage from './pages/DetailOrderPage/DetailOrderPage'
import FavouritePage from './pages/FavouritePage/FavouritePage'
import ListBrandByCategory from './pages/ListBrandByCategory'
import AddNewCategory from './pages/Admin/ManageCategory/AddNewCategory'
import PageListOrder1 from './pages/ListOrder/PageListOrder1'
import ManageCoupon from './pages/Admin/ManageCoupon/ManageCoupon'
import AddNewCoupon from './pages/Admin/ManageCoupon/AddNewCoupon'
import DashboardPage from './pages/Admin/Dashboard/DashboardPage'
import UpdateProduct from './pages/Admin/ManageProduct/UpdateProduct'
// import AdminTable from './pages/Admin/AdminTable'

export default function useRouteElements() {
  const routeElements = useRoutes([
    {
      path: '/',
      element: (
        <MainLayout title='Trang chủ'>
          <HomePage />
        </MainLayout>
      )
    },
    {
      path: '/dang-nhap',
      element: (
        // <RegisterLayout titileHeader='Đăng nhập'>
        //   <Login />
        // </RegisterLayout>
        <MainLayout title='Đăng nhập'>
          <Login />
        </MainLayout>
      )
    },
    {
      path: '/dang-ki',
      element: (
        // <RegisterLayout titileHeader='Đăng ký'>
        //   <Register />
        // </RegisterLayout>
        <MainLayout title='Đăng kí'>
          <Register />
        </MainLayout>
      )
    },
    {
      path: '/kich-hoat-tai-khoan',
      element: (
        // <RegisterLayout titileHeader='Kích hoạt tài khoản'>
        //   <ActiveAccount />
        // </RegisterLayout>
        <MainLayout title='Xác thực tài khoản'>
          <ActiveAccount />
        </MainLayout>
      )
    },
    {
      path: '/xac-thuc',
      element: (
        // <RegisterLayout titileHeader='Kích hoạt tài khoản'>
        //   <ConfirmActive />
        // </RegisterLayout>
        <MainLayout title='Xác thực code'>
          <ConfirmActive />
        </MainLayout>
      )
    },
    {
      path: '/danh-sach/:category',
      element: (
        <MainLayout title='Danh sách'>
          <ProductList />
        </MainLayout>
      )
    },
    {
      path: '/tim-kiem',
      element: (
        <MainLayout title='Tìm kiếm'>
          <SearchPage />
        </MainLayout>
      )
    },
    {
      path: '/san-pham/:id',
      element: (
        <MainLayout title='Chi tiết'>
          <ProductDetail />
        </MainLayout>
      )
    },
    {
      path: '/laptop/:id',
      element: (
        <MainLayout title='Chi tiết'>
          <ProductDetail />
        </MainLayout>
      )
    },
    {
      path: '/phu-kien/:id',
      element: (
        <MainLayout title='Chi tiết'>
          <ProductDetail />
        </MainLayout>
      )
    },
    {
      path: '/smart-watch/:id',
      element: (
        <MainLayout title='Chi tiết'>
          <ProductDetail />
        </MainLayout>
      )
    },
    {
      path: '/tablet/:id',
      element: (
        <MainLayout title='Chi tiết'>
          <ProductDetail />
        </MainLayout>
      )
    },
    {
      path: '/quen-mat-khau',
      element: (
        <MainLayout title='Quên mật khẩu'>
          <ForgotPassword />
        </MainLayout>
      )
    },
    {
      path: '/reset-password',
      element: (
        <MainLayout title='Đặt lại mật khẩu'>
          <ResetPassword />
        </MainLayout>
      )
    },
    {
      path: '/gio-hang',
      element: (
        <MainLayout title='Giỏ hàng'>
          <ListCart />
        </MainLayout>
      )
    },
    {
      path: '/trang-ca-nhan',
      element: (
        <MainLayout title='Trang cá nhân'>
          <ProfilePage />
        </MainLayout>
      )
    },
    {
      path: '/yeu-thich',
      element: (
        <MainLayout title='Yêu thích'>
          <FavouritePage />
        </MainLayout>
      )
    },
    {
      path: '/thanh-toan/thanh-cong',
      element: (
        <MainLayout title='Thanh toán thành công'>
          <PaymentSuccess />
        </MainLayout>
      )
    },
    {
      path: '/dien-thoai',
      element: (
        <MainLayout title='Điện thoại'>
          <ListBrandByCategory />
        </MainLayout>
      )
    },
    {
      path: '/laptop',
      element: (
        <MainLayout title='Danh sách laptop'>
          <ListBrandByCategory />
        </MainLayout>
      )
    },
    {
      path: '/phu-kien',
      element: (
        <MainLayout title='Danh sách phụ kiện'>
          <ListBrandByCategory />
        </MainLayout>
      )
    },
    {
      path: '/smartwatch',
      element: (
        <MainLayout title='Danh sách Smartwatch'>
          <ListBrandByCategory />
        </MainLayout>
      )
    },
    {
      path: '/tablet',
      element: (
        <MainLayout title='Danh sách Tablet'>
          <ListBrandByCategory />
        </MainLayout>
      )
    },
    {
      path: '/thanh-toan/that-bai',
      element: (
        <MainLayout title='Thanh toán thất bại'>
          <PaymentFail />
        </MainLayout>
      )
    },
    {
      path: '/don-hang',
      element: (
        <MainLayout title='Đơn hàng'>
          <PageListOrder1 />
        </MainLayout>
      )
    },
    {
      path: '/don-hang/chi-tiet-don-hang/:id',
      element: (
        <MainLayout title='Chi tiết đơn hàng'>
          <DetailOrderPage />
        </MainLayout>
      )
    },
    {
      path: '/admin/dashboard',
      element: (
        <AdminLayout title={'Trang tổng quan'}>
          <DashboardPage />
        </AdminLayout>
      )
    },
    {
      path: '/admin/manage-coupon',
      element: (
        <AdminLayout title={'Quản lý voucher'}>
          <ManageCoupon />
        </AdminLayout>
      )
    },
    {
      path: '/admin/manage-coupon/add-new',
      element: (
        <AdminLayout title={'Thêm mới coupon'}>
          <AddNewCoupon />
        </AdminLayout>
      )
    },
    {
      path: '/admin/manage-users',
      element: (
        <AdminLayout title={'Quản lý người dùng'}>
          <ManageUser />
        </AdminLayout>
      )
    },
    {
      path: '/admin/manage-categories',
      element: (
        <AdminLayout title={'Quản lý danh mục'}>
          <ManageCategory />
        </AdminLayout>
      )
    },
    {
      path: '/admin/manage-categories/add-new',
      element: (
        <AdminLayout title={'Add new category'}>
          <AddNewCategory />
        </AdminLayout>
      )
    },
    {
      path: '/admin/manage-brands',
      element: (
        <AdminLayout title={'Quản lý thương hiệu'}>
          <ManageBrand />
        </AdminLayout>
      )
    },
    {
      path: '/admin/manage-brand/add-new',
      element: (
        <AdminLayout title={'Add new brand'}>
          <AddNewBrand />
        </AdminLayout>
      )
    },
    {
      path: '/admin/manage-products',
      element: (
        <AdminLayout title={'Quản lý sản phẩm'}>
          <ManaegProduct />
        </AdminLayout>
      )
    },
    {
      path: '/admin/manage-products/add-new',
      element: (
        <AdminLayout title={'Thêm mới sản phẩm'}>
          <AddNewProduct />
        </AdminLayout>
      )
    },
    {
      path: '/admin/manage-products/update',
      element: (
        <AdminLayout title={'Cập nhật sản phẩm'}>
          <UpdateProduct />
        </AdminLayout>
      )
    },
    {
      path: '/admin/manage-orders',
      element: (
        <AdminLayout title={'Manage Order'}>
          <ManageOrder />
        </AdminLayout>
      )
    },
    {
      path: '/admin/manage-orders/:id',
      element: (
        <AdminLayout title={'Chi tiết đơn hàng'}>
          <DetailOrderPage />
        </AdminLayout>
      )
    },

    {
      path: '*',
      element: (
        <MainLayout title='404'>
          <Page404 />
        </MainLayout>
      )
    }
  ])
  return routeElements
}
