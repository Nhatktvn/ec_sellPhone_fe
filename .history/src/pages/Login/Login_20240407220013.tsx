...
import { login, logout, roleAuthen } from '../../slices/authSlice'
...
export default function Login() {
  ...
  const roleAuthen = useSelector((state: RootState) => state.auth.roleAuthenticate)
  const schema = yup
    .object({
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      const roleAuth = await getRoleAuth()
      if (roleAuth && roleAuth.data) {
        console.log(roleAuth.data)

        dispatch(roleAuthen(roleAuth.data))
      }
      navigation('/')
      ...
      if (error.response?.status === 422) {
        Object.keys(dataError).forEach((key) => {
          setError(key as keyof { username: string; password: string }, {
            message: dataError[key],
            type: 'Server'
          })
        })
      }
          ...
          <div className='lg:col-span-2 lg:col-start-4'>
            <form onSubmit={onSubmit} className='p-10 rounded bg-white shadow-sm'>
              ...
              <button
                ...
                className={`w-full p-3 bg-orange text-white font-medium mt-5 rounded-sm active:bg-[#e2492b] ${!watch().username || !watch().password ? 'cursor-not-allowed opacity-[0.7]' : 'cursor-pointer hover:opacity-[0.9]'}`}
              >
                ĐĂNG NHẬP
              </button>
