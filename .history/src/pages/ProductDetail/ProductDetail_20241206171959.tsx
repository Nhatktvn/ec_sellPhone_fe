...
const ProductDetail = () => {
  ...
  const [storageSelect, setStorageSelect] = useState<storagePrice>()
  ...
  const handleGetAvalable = async () => {
    try {
      const data = {
        color: colorSelect,
        ram: storageSelect?.ram,
        storage: storageSelect?.storage,
  ...
  const handleGetVariant = () => {
        ...
        product?.variantDTOList.map((variant) => {
          if (variant.color === color) {
            const objStoragePrice: storagePrice = {
              ram: variant.ram,
              storage: variant.storageCapacity,
    ...
    const colorFist: storagePrice = listStorageByColor && listStorageByColor[0]
    console.log(colorFist)

    setListStorage(listStorageByColor)
    setStorageSelect(colorFist)
  }
  ...
  const handleClickColor = (color: string) => {
    ...
    listStorageByColor && setStorageSelect(listStorageByColor[0])
    setListStorage(listStorageByColor)
  }
        ...
        <div className='container'>
          <div className='grid grid-cols-12 bg-white rounded-xl p-4 my-4'>
            ...
            <div className='col-span-12 md:col-span-6 lg:col-span-7 mt-5 relative pl-5 flex items-center justify-center'>
              ...
              <div>
                <h3 className='text-3xl'>{`${product == undefined ? '' : product.name}`}</h3>
                      ...
                      listStorage.map((storage, idx) => {
                        ...
                        return (
                          <label
                            ...
                            className={`cursor-pointer text-center p-2 text-sm rounded-sm border ${storage === storageSelect && 'border-orange'}`}
                          >
                            <div className='font-semibold'>
                              <input
                                checked={storage === storageSelect}
                                type='radio'
                                name='storage'
                                className='hidden'
                                id={`${storage.storage}`}
                                onChange={() => setStorageSelect(storage)}
                              />
                              {`${storage.ram}GB - ${storage.storage}GB`}
                            </div>
                ...
                <div className='grid grid-cols-12 mt-7 ml-3'>
                  ...
                  <div className='col-span-10 flex-col items-center'>
                    ...
                    {!availableProduct ? (
                      <span className='text-sm text-red-500 font-bold block mt-3'>Hết hàng</span>
                    ) : (
                      availableProduct > 0 &&
                      availableProduct <= 20 && (
          ...
          <div className='grid grid-cols-12 gap-3'>
            <div className='col-span-8 bg-white rounded-xl'>
              ...
              {selectSpecOrReview == 'review' ? (
                ...
                <div className='col-span-5 rounded-xl bg-white py-3 px-10 h-max flex flex-col items-center pb-10'>
                  <div className='w-full p-5'>
                    <img src={product.specificationDTO.urlImageSpec} alt='' />
                  </div>
                  <Specifacation product={product} />
                </div>
              )}
            </div>
            <div className='col-span-4 bg-white px-3 rounded-md py-5 h-max '>
              ...
              <div className='flex flex-col gap-5'>
                ...
                <div className='flex gap-2 items-center text-gray-600 text-sm'>
                  ...
                  <span>Bộ sản phẩm gồm: Hộp, Sách hướng dẫn, Cáp sạc, Củ sạc nhanh rời, Phụ kiện, Quà tặng</span>
                </div>
                <div className='flex gap-2 items-center text-gray-600 text-sm'>
