        ...
        {listCategories &&
          listCategories.map((c: any, idx) => {
            if (idCateSelect == c.id) {
              return <li className='font-bold text-xl border-b-4 border-blue-600 text-blue-600'>{c.name}</li>
            }
            return (
              <li onClick={() => setIdCateSelect(c.id)} className='font-bold text-xl border-b-4'>
                {c.name}
              </li>
            )
          })}
