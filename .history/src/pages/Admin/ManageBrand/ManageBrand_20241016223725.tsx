...
import { deleteCategoryById, getBrandByCategoryName, getBrands } from '../../../apis/brand.api'
...
const App: React.FC = () => {
  ...
  const [chooseCate, setChooseCate] = useState<string>('')
  ...
  const getBrandByCate = async () => {
    try {
      const fetchBrand = await getBrandByCategoryName(chooseCate)
      if (fetchBrand.status == 200) {
        setData(fetchBrand.data)
      }
    } catch (error) {
      console.log(error)
    }
  ...
  const deleteCategory = async (record: Partial<product> & { id: number }) => {
    ...
    console.log(id)
    try {
      const rsDeleteCate = await deleteCategoryById()
  ...
  return (
    <div>
        ...
        <ul className='flex gap-5 text-lg'>
          ...
          {listCate &&
            ...
            listCate.map((cate: any, idx) => {
              if (chooseCate == cate.name) {
                console.log('ok')
                return (
                  <li key={cate.id} className='p-2 cursor-pointer font-bold text-blue-500 border-b-4 border-blue-500'>
                    {cate.name}
                  </li>
                )
              }
              return (
                <li
                  onClick={() => setChooseCate(cate.name)}
                  key={cate.id}
                  className='hover:bg-gray-200 rounded-md p-2 cursor-pointer font-bold'
                >
                  {cate.name}
                </li>
              )
      ...
      <div className='float-end'>
        <button
          onClick={() => navigation('/admin/manage-brand/add-new')}
