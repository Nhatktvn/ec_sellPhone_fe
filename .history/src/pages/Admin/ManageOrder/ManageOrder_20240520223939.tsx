...
const ManageOrder = () => {
  ...
  const handleUpdateStatusOrder = async (orderId: number, statusOrderName: string) => {
    try {
      ...
      formData.append('statusOrderName', statusOrderName)
      const rsUpdate = await updateStatusOrderByAdmin(formData)
              ...
              return (
                <tr className={idx % 2 == 0 ? 'bg-gray-300' : ''} key={order.id}>
                  ...
                  <td className='flex gap-4 justify-center bg-white'>
                    ...
                    <select defaultValue={order.statusOrder} className='cursor-pointer'>
