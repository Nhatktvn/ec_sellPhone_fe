...
const AddNewProduct = () => {
  ...
  const handleRemoveVariant = (idx: number) => {
    console.log(idx)
    const arrayTmp: variant[] | undefined = listVariant && [...listVariant]
    const newArray = arrayTmp?.splice(idx + 1, 1)
    setListVariant(newArray)
  }
          ...
          <div className='w-full flex gap-2 items-center'>
            {listVariant &&
              listVariant.map((variant, idx) => {
                return (
                  <div className='border-2 p-2 rounded-md border-orange relative group' key={idx}>
                    <button
                      className='text-red-600 font-bold p-1 absolute hidden group-hover:block top-0 right-1'
            ...
            {!isInputVariant ? (
              ...
              <form
                ...
                className='w-max border-2 border-gray-400 rounded-md flex flex-col gap-1 p-2'
              >
                <input
