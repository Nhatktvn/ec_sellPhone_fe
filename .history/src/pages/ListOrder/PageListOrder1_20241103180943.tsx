...
const PageListOrder1 = () => {
  ...
  const renderTextStatus = (status: string) => {
    if (status != 'read_to_pick' && status != 'return') {
      return <div className={`${colorTextStatusOrder.other}`}>{status}</div>
    }
    return <div className={`${colorTextStatusOrder[status]}`}>{status}</div>
  }
  useEffect(() => {
