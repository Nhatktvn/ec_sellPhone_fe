...
import { getListOrder, putCancelOrder, putReceivedOrder } from '../../apis/order.api'
...
const PageListOrder1 = () => {
      ...
      sortOrders.map(async (order: any, idx: any) => {
        ...
        try {
          ...
          if (apiGetStatusOrder && apiGetStatusOrder.status == 200) {
            ...
            if(apiGetStatusOrder.data.data.status == "delivered" && order.statusOrder !== )
          }
        } catch (error) {
          console.log(error)
        }
  ...
  const handleRecivedOrder = async (id: number) => {
    try {
      const apiReceived = await putReceivedOrder(id)
