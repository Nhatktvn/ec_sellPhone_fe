...
export default function ListCart() {
  ...
  const handleChangeQuantity = async (idCartItem: number, quantity: number) => {
    console.log('Ok')

    if (quantityTmp == 0) {
      setQuantityTmp(quantity)
    } else {
      setQuantityTmp((prev) => prev + 1)
    }
    ...
    typingTimeoutRef.current = setTimeout(() => {
      ...
      hanndleFetchUpdateQuantity(idCartItem)
    }, 1000)
  }

  const hanndleFetchUpdateQuantity = async (idCartItem: number) => {
    try {
      const dataBody = { idCartItem, quantity: quantityTmp }
                          ...
                          <div className='col-span-3'>
                            ...
                            <div>
                              ...
                              <button
                                ...
                                onClick={() => handleChangeQuantity(item.id, item.quantity)}
                              >
                                +
                              </button>
                            </div>
