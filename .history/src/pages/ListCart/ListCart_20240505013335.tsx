...
interface address {
  provinceName: dataProvince | null
  districtName: dataDistrict |null
  wardName: dataWard | null
}
export default function ListCart() {
  ...
  const [addressDelivery, setAddressDelivery] = useState<address>({ districtName: null, provinceName: null, wardName: null })
  ...
  const handleChangeAddress = async (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (addressDelivery.wardName) {
      try {
        ...
        const rsFee = await getFee(dataGetFee)
      } catch (error) {
        console.log(error)
      }
    }
