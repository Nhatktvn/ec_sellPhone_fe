  ...
  const getProvince = async () => {
    try {
      ...
      if (rsGetProvince && rsGetProvince.status == 200) {
        const dataProvince = rsGetProvince.data.data.sort(function (a: province, b: province) {
      ...
      <div className='lg:col-span-5 h-full bg-white rounded-xl p-3'>
        ...
        <div>
          <select id='province' onChange={handleChangeProvince}>
            {listProvince &&
              listProvince.map((p: province, idx) => <option value={p.ProvinceID}>{p.ProvinceName}</option>)}
          </select>
        </div>
      </div>
    </div>
