...
import { sendMailConfirm } from '../../apis/email.api'
...
export default function ListCart() {
  ...
  const handleOrder = async (codeOrder: string) => {
    try {
      ...
      const rsOrderCod = await orderCod({
        ...
        phone: phoneDelivery,
        name: nameDelivery,
        couponId: couponChoose ? couponChoose?.id : null
      })
      if (rsOrderCod && rsOrderCod.status === 201) {
        toast.success('Thanh toán thành công')
        getCountCart()
      }
      const fetchMailSendConfirm = await sendMailConfirm()
      console.log(fetchMailSendConfirm)
  ...
  const handleOrderOfVNPay = async () => {
    try {
      ...
      formData.append('couponId', JSON.stringify(couponChoose ? couponChoose?.id : 0))
                      ...
                      <div className='w-full'>
                        ...
                        <div className='flex mt-2 justify-between'>
                          <div className='gap-4 text-sm grid grid-cols-12'>
                            ...
                            <div className='col-span-3'>
                              <p>Ram</p>
                              <div className='py-1 px-2 rounded-full border w-max'>{item.ram}</div>
                            </div>
                            <div className='col-span-3'>
                              <p>Dung lượng</p>
