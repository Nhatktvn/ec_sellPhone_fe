...
export default function ListCart() {
  ...
  return (
    <div className='container grid grid-cols-12 gap-3 mt-5'>
      ...
      {isAuthenticated && cartItems && cartItems?.length > 0 && (
        <div className='lg:col-span-5 h-full bg-white rounded-xl p-5'>
          ...
          <div className='flex gap-5 mt-5 items-center'>
            <h3>Voucher: </h3>
            <button className='border border-gray-500 py-2 px-3 rounded-lg active:scale-90 duration-150'>
              Chọn voucher
            </button>
          </div>
          <div className='mt-5'>
      ...
      {showModalCoupon && <ModalListCoupon setShowModal={setShowModalCoupon} />}
    </div>
  )
}
