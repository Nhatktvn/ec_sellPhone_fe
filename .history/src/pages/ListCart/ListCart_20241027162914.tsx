...
import { getServiceDelivery } from '../../apis/GHN.api'
...
export default function ListCart() {
  ...
  console.log('code', addressCodeDelivery)
  ...
  const FetchServiceDelivery = async () => {
    try {
      ...
      if (fetchService && fetchService.status === 200) {
        setListServiceDelivery(fetchService.data)
      }
    } catch (error) {
      console.log(error)
    }
          ...
          <div className='mt-5'>
            ...
            >
              {listServiceDelivery &&
                ...
                listServiceDelivery.map((s: any, idx) => <option value={s.service_id}>{s.short_name}</option>)}
            </select>
          </div>
          <div className='mt-5'>
