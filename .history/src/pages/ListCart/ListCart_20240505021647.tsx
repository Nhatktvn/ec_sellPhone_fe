...
export default function ListCart() {
  ...
  const handleChangeAddress = async (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (addressCodeDelivery.provinceName && addressCodeDelivery.wardName) getCodeAddress()
  }
  const getCodeAddress = async () => {
    try {
      const dataGetFee = {
        ...
        to_district_id: Number(addressCodeDelivery.districtName),
        to_ward_code: addressCodeDelivery.wardName,
        height: 20,
        length: 20,
        weight: 2000,
        width: 20,
