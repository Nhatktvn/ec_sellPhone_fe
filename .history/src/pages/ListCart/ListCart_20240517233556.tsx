...
export default function ListCart() {
  ...
  const handleOrder = async () => {
    try {
      ...
      formData.append('address', addressTmp)
      formData.append('name', nameDelivery)
      formData.append('phone', phoneDelivery)
      console.log(addressTmp)
      if (typePayment === 'cod') {
        const rsOrderCod = await orderCod({ address: addressTmp, phone: phoneDelivery, name: nameDelivery })
        ...
        const rsOrderVnpay = await orderVnpay(formData, 10000)
        if (rsOrderVnpay) {
