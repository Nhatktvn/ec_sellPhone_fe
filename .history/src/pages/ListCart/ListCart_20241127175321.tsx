...
export default function ListCart() {
  ...
  const caclTotalPrice = () => {
    const tmp =
      cartItems?.reduce((accumulator, currentValue) => {
        return accumulator + currentValue.sellPrice * currentValue.quantity
      }, 0) + fee
    const valueCoupon = couponChoose
      ? couponChoose.typeCoupon == 'fixed'
        ? couponChoose.couponValue
        : (tmp * couponChoose.couponValue) / 100
      : 0
    return tmp - valueCoupon
  }
  return (
    <div className='container grid grid-cols-12 gap-3 mt-5'>
        ...
        <div className='lg:col-span-5 h-full bg-white rounded-xl p-5'>
          ...
          <div className='flex gap-5 mt-5 items-center'>
            ...
            {!couponId ? (
              ...
              listCoupon.map((coupon: Coupon) => {
                if (coupon.id == couponId) {
                  setCouponChoose(coupon)
                  return (
                    <button
                      onClick={() => setShowModalCoupon(true)}
                      className='border border-gray-500 py-2 px-3 rounded-lg active:scale-90 duration-150'
                    >
                      {coupon.typeCoupon == 'fixed'
                        ? `Giảm ${formatToVND(coupon.couponValue)}`
                        : `Giảm ${coupon.couponValue} %`}
                    </button>
                  )
                }
                return null
          ...
          <div className='mt-5'>
            ...
            <p className='flex justify-between text-2xl font-bold'>
              Thanh toán:{' '}
              <span className='text-orange'>{cartItems ? formatToVND(caclTotalPrice()) : formatToVND(0)}</span>
            </p>
          </div>
          <button
      ...
      {showModalCoupon && (
        <ModalListCoupon setShowModal={setShowModalCoupon} setCouponId={setCouponId} coupons={listCoupon} />
      )}
    </div>
  )
}
