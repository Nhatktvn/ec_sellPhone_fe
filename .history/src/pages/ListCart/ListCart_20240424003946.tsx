...
import { getAllDitrict, getAllProvince } from '../../apis/address.api'
...
export default function ListCart() {
  ...
  const [listProvince, setListProvince] = useState<dataAddress[]>([])
  const [listDistrict, setListDistrict] = useState<dataAddress[]>([])
  const [addressDelivery, setAddressDelivery] = useState<address | undefined>()
  ...
  const getProvince = async () => {
    try {
      ...
      if (rsGetProvince && rsGetProvince.status == 200) {
        const dataProvince = rsGetProvince.data.data.sort(function (a: dataAddress, b: dataAddress) {
  ...
  const getDistrict = async (provinceId: number) => {
    try {
      const rsGetDistrict = await getAllDitrict({ province_id: provinceId })
      if (rsGetDistrict && rsGetDistrict.status == 200) {
        const dataDistrict = rsGetDistrict.data.data.sort(function (a: dataAddress, b: dataAddress) {
          return a.name.localeCompare(b.name)
        })
  ...
  return (
    <div className='container grid grid-cols-12 gap-3 mt-5'>
      ...
      <div className='lg:col-span-5 h-full bg-white rounded-xl p-3'>
        ...
        <div>
          <select id='province' onChange={handleChangeProvince}>
            {listProvince &&
              listProvince.map((p: dataAddress) => (
