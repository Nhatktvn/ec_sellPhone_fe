...
import { getSearchProduct } from '../../apis/product.api'
...
export default function SearchPage() {
    ...
    if (listStorageChoose && listStorageChoose.length > 0) {
      listProductFilter = listProductFilter.filter((product) => {
        ...
        for (let i = 0; i < product.variantDTOList.length; i++) {
          if (listStorageChoose.includes(product.variantDTOList[i].storageCapacity.toString())) {
            bool = true
            break
          }
        }
  ...
  const getProductsByCateName = async () => {
    try {
      const rsGetProductByCate = await getSearchProduct(key)
