...
export default function ProductList() {
  ...
  const [productsFilter, setProductFilter] = useState<product[] | null>([])
  ...
  const handleFilterProduct = () => {
    let listProductFilter: any = []
    if (listPriceChoose) {
      console.log('Okaaaaa')

      listPriceChoose?.map((priceRange) => {
        let rsFilter: any[] | null = []
        rsFilter = handleFilterProductByPrice(priceRange)
        listProductFilter = listProductFilter?.concat(rsFilter)
      })
    } else {
      listProductFilter = products
    }
