...
import { getProductsByBrand } from '../../apis/product.api'
...
export default function ProductList() {
  ...
  const [listStorageChoose, setListStorageChoose] = useState<number[]>([])
  const [sortDir, setSortDir] = useState<string>('priceDefault')
  useEffect(() => {
  ...
  useEffect(() => {
    handleFilterProduct()
    // handleSortProducts(sortDir)
  }, [listPriceChoose, listStorageChoose])
  useEffect(() => {
    handleSortProducts(sortDir)
  }, [sortDir])
  const handleFilterProduct = () => {
    setSortDir('priceDefault')
  ...
  const handleFilterProductByPrice = (priceRange: string) => {
    ...
    switch (priceRange) {
      case 'under2m':
        filtered = products && products.filter((product) => product.variantDTOList[0].sellPrice < 2000000)
        break
      case '2mTo4m':
        filtered =
          products &&
          products.filter(
            (product) => product.variantDTOList[0].sellPrice > 2000000 && product.variantDTOList[0].sellPrice < 4000000
          )
        break
      case '4mTo7m':
        filtered =
          products &&
          products.filter(
            (product) => product.variantDTOList[0].sellPrice > 4000000 && product.variantDTOList[0].sellPrice < 7000000
          )
        break
      case '7mTo13m':
        filtered =
          products &&
          products.filter(
            (product) => product.variantDTOList[0].sellPrice > 7000000 && product.variantDTOList[0].sellPrice < 13000000
          )
        break
      case 'above13m':
        filtered = products && products.filter((product) => product.variantDTOList[0].sellPrice > 13000000)
        break
    }
    return filtered
  }
  ...
  const handleChangeInputStorage = (e: ChangeEvent<HTMLInputElement>) => {
    const { checked, value } = e.target
    if (checked) {
      setListStorageChoose([...listStorageChoose, Number(value)])
    } else {
      setListStorageChoose(listStorageChoose.filter((price) => price !== Number(value)))
    }
  }

  const getListCategory = async () => {
    try {
      const rsGetCategory = await getAllCategories()
  ...
  const getProductsByCateName = async () => {
    try {
      ...
      const rsGetProductByCate = await getProductsByBrand(category)
  ...
  const handleSortProducts = (sortDir: string) => {
    switch (sortDir) {
      case 'priceDefault':
        handleFilterProduct()
        break
      case 'priceDownToUp':
        const sortedProducts = [...productsFilter].sort(
          (a, b) => a.variantDTOList[0].sellPrice - b.variantDTOList[0].sellPrice
        )
        setProductsFilter(sortedProducts)
        break
      case 'priceUpToDown':
        const sortedProductss = [...productsFilter].sort(
          (a, b) => b.variantDTOList[0].sellPrice - a.variantDTOList[0].sellPrice
        )
        setProductsFilter(sortedProductss)
        break
      default:
        break
    }
            ...
            <div className='text-sm grid grid-cols-2 gap-2'>
              <label htmlFor='32GB' className='flex gap-2 items-center cursor-pointer'>
                <input
                  ...
                  name='storage'
                  value='32GB'
                  checked={listStorageChoose.includes(32)}
                  id='32GB'
              ...
              <label htmlFor='64GB' className='flex gap-2 items-center cursor-pointer'>
                <input
                  ...
                  value='64GB'
                  id='64GB'
                  checked={listStorageChoose.includes(64)}
                  onChange={handleChangeInputStorage}
                />
                <span>64GB</span>
              </label>
              <label htmlFor='128GB' className='flex gap-2 items-center cursor-pointer'>
                <input
                  ...
                  value='128GB'
                  id='128GB'
                  checked={listStorageChoose.includes(128)}
                  onChange={handleChangeInputStorage}
                />
                <span>128GB</span>
              </label>
              <label htmlFor='256GB' className='flex gap-2 items-center cursor-pointer'>
                <input
                  ...
                  value='256GB'
                  id='256GB'
                  checked={listStorageChoose.includes(256)}
                  onChange={handleChangeInputStorage}
                />
                <span>256GB</span>
              </label>
              <label htmlFor='512GB' className='flex gap-2 items-center cursor-pointer'>
                <input
                  ...
                  value='512GB'
                  id='512GB'
                  checked={listStorageChoose.includes(512)}
                  onChange={handleChangeInputStorage}
                />
                <span>512GB</span>
              </label>
              <label htmlFor='1TB' className='flex gap-2 items-center cursor-pointer'>
                <input
                  ...
                  value='1TB'
                  id='1TB'
                  checked={listStorageChoose.includes(1)}
                  onChange={handleChangeInputStorage}
                />
                <span>1TB</span>
              </label>
        ...
        <div className='col-span-12 md:col-span-9'>
          <div className='flex gap-3 text-sm py-3 mx-3 bg-[#EDEDED] items-center pl-2'>
            ...
            <div className='grid grid-cols-12 gap-3 mr-2'>
              ...
              >
                <option value='priceDefault'>Giá: Mặc định</option>
