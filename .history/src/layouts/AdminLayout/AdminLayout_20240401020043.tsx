...
const AdminLayout: React.FC<Props> = ({ children, title }) => {
  ...
  return (
    <Layout className='relative'>
      ...
      >
        <div className='demo-logo-vertical my-5 flex items-center gap-5 w-[200px] justify-center'>
          <div className='w-11 h-11  bg-red-400 rounded-full'>
            <img src='' alt='' />
          </div>
          <h3 className={`${collapsed ? 'opacity-100' : 'opacity-0'} text-white duration-300`}>Welcome, Admin</h3>
        </div>
        <Menu
          theme='dark'
          mode='inline'
